...
| Stale or replayed Guardian decision | Elevation, repudiation, tampering | Expired, future-effective, stale, replayed, or wrong-scope Guardian decision authorizes work | Guardian expiry/replay policy, one-time decision nonce, decision scope hash, mock replay verifier, fail-closed invariants | Guardian replay check result, stale/expired decision denial, scope mismatch, repeated nonce | `guardian.decision`, `guardian.replay`, evidence artifact, incident review input | schema_defined |
| Replay-store atomicity failure or outage | Elevation, repudiation, denial | One-time Guardian/approval nonce can be consumed ambiguously or reused | `replay.store.record` fail-closed states, denial-path evidence, blocked authorization | `failed_closed` atomicity, duplicate nonce consume attempts, replay-denied without evidence | Replay-store record, replay artifact, denial evidence, incident review input | schema_defined |
...
| Connector consent or scope drift | Elevation, disclosure | Connector scope expands without consent/revocation/object-property authorization evidence | Connector consent policy, scope review, connector readiness gates, connector reconciliation linkage, live access false, revocation evidence | Scope mismatch, revoked consent, missing object/property auth mapping, prompt-injection unresolved | Connector consent, connector trust, connector readiness, connector scope review, connector reconciliation, taint evidence | schema_defined |
...
| Unsafe Supervisor subscription turn | `supervisor.conversation.turn`, `model.route`, `guardian.decision`, `evidence.artifact` | Accept only explicitly confirmed public or non-sensitive internal text; require clean low-risk Supervisor-only routing, read-only ephemeral execution, no user config/rules/web/tools/memory/helpers/fallback/Arc dispatch, pre/post evidence, zero tool events, and no raw conversation text in durable evidence. |
| Proposal acceptance confused with execution approval | `supervisor.task.proposal`, `guardian.decision`, harness evidence | Use the explicit `accepted_for_future_approval` state; keep approval token, worker assignment, dispatch, tools, connectors, submission, and external effects false; require expected revisions and atomically commit proposal state with completion evidence. |
| Approval preview confused with an approval request or result | `supervisor.approval.preview`, `guardian.decision`, harness evidence | Keep preview in a separate contract/store; bind exact source revision/hash; expose identity, expiry, replay, and scope requirements while fixing request/result/binding/token/replay/dispatch flags false; prohibit silent promotion and atomically evidence each foreground transition. |
...
| Personal-PC lab session mistaken for production identity | `operator.session.binding`, `guardian.decision`, `approval.request`, `evidence.artifact` | The binding is pseudonymous, process-bound, short-lived, no-PIN/no-MFA, lost on restart, and explicitly cannot approve or authorize execution. It may create only a fixed synthetic `pending_review` request after source and fresh-intent checks. |
| Pending or negative result mistaken for approval authority | `approval.request`, `approval.result`, `guardian.decision`, harness evidence | The synthetic lane permits only pending, denied, cancelled, or expired states; exact request hash and legal transition are rechecked; request/result/completion evidence commit atomically; token/binding/nonce/replay/worker/dispatch/side-effect fields stay null or false; no positive approval endpoint exists. |
| Readiness assessment mistaken for implementation authorization | `approval.readiness`, governance and approval contract refs, evidence metadata | The assessment is design-only; `implementation_authorized` and every result/token/binding/replay/dispatch/external-effect flag are schema-fixed false. Even `ready_for_implementation_review` requires a separate explicit implementation decision. |
...
| Connector consent or scope drift | `governance.connector_consent`, `connector.trust`, `connector.readiness`, `connector.scope_review`, `connector.reconciliation`, `taint.ref`, `evidence.artifact` | Consent/scope/readiness/trust/revocation records must reconcile fail-closed; live access false, secret material absent, blocked scopes explicit, object/property authorization mapped, revocation evidenced, and prompt-injection review required before any future live-review lane. |
| Connector acceptance-score or reconciliation cadence drift | `connector.acceptance_score`, `connector.reconciliation_slo`, `connector.provider_profile`, `connector.readiness`, `connector.reconciliation`, `connector.revocation_drill`, `console.alert`, `supervisor.health` | Pending/missed revocation propagation, stale/missed cadence, missing source-of-truth evidence, or failed dimensions must fail closed and prevent lab-ready posture claims. |
| Connector ownership or escalation accountability drift | `connector.ownership`, `connector.escalation`, `connector.provider_profile`, `connector.readiness`, `connector.acceptance_score`, `connector.reconciliation_slo`, `console.alert`, `supervisor.health` | Missing/stale/conflicted ownership, SoD violations, or revocation/disable ownership gaps must fail closed and surface escalation evidence before any lab-ready posture is represented. |
| Connector defaults/SLO-target/threshold drift | `connector.defaults`, `connector.slo_target`, `connector.score_threshold`, `connector.readiness`, `connector.acceptance_score`, `console.alert`, `supervisor.health` | Missing defaults/targets/thresholds, blocked-MVP category override, or outbound-default ambiguity must fail closed and remain non-authorizing. |
...
| Rogue helper agent | `helper.scope`, `helper.assignment`, `helper.result`, `task.execution`, `tool.invocation`, `memory.access`, `guardian.decision`, `incident.ops`, `evidence.artifact` | The implemented Office Helper accepts only fixed synthetic scenario IDs and deterministic foreground review; model/tool/file/memory/approval/connector/Arc/submission authority is false. Other helper agents remain contract-only, supervisor-side actors; out-of-scope requests are denied and evidenced. |
...
| Stale or replayed Guardian decision | `guardian.decision`, `guardian.replay`, `approval.binding`, `token.verification`, `task.execution`, `tool.invocation`, `evidence.artifact` | Missing expiry, ambiguous timestamp, future-effective time beyond skew, stale age, replayed nonce, scope mismatch, approval-binding mismatch, token-verification mismatch, taint, or blocked-MVP action fails closed. |
| Replay-store atomicity failure or ambiguity | `replay.store.record`, `guardian.replay`, `guardian.decision`, `approval.binding`, `token.verification`, `evidence.artifact`, `evidence.failure` | Duplicate consume, missing denial evidence, failed-closed atomicity, or replay-store outage must deny authorization and produce denial-path evidence. |
...
- What final source-of-truth owner, escalation owner, and SoD separation values
  should be mandatory per connector category before any future implementation lane?
- What final connector default values, override controls, and SLO/threshold
  target values should replace metadata placeholders before any future
  implementation lane?
